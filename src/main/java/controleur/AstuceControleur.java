...
public class AstuceControleur {
	...
	public ModelAndView detailChat(@PathVariable("idAstuce") int idAstuce, HttpSession httpSession){
    	...
    	System.out.println("nombre de like total :"+ nbVoteTotal+" nombre de dislike"+ nbDislike+" nombre de like :" + nblike);
		...
		if(email==null){
	    	...
	    	List<Vote> votes = (List<Vote>)criteria.list();
				if(votes==null||votes.size()==0){
					info="Vous n'avez pas encore votÃ© pour cette astuce";
					cas="1";
					modelAndView.addObject("Admin",httpSession.getAttribute("Admin"));
					modelAndView.addObject("email",httpSession.getAttribute("emailUser"));
					modelAndView.addObject("Astuce",astuce);
				...
				if(vote.getValeur().equals("2")){
					info="Vous avez likÃ© cette astuce";
					cas=vote.getValeur();
					modelAndView.addObject("Admin",httpSession.getAttribute("Admin"));
					modelAndView.addObject("email",httpSession.getAttribute("emailUser"));
					modelAndView.addObject("Astuce",astuce);
				...
				if(vote.getValeur().equals("3"))
					info="Vous avez dislikÃ© cette astuce";
					cas=vote.getValeur();
					modelAndView.addObject("Admin",httpSession.getAttribute("Admin"));
					modelAndView.addObject("email",httpSession.getAttribute("emailUser"));
					modelAndView.addObject("Astuce",astuce);
	...
	public ModelAndView RechAstucesChat(HttpSession httpSession,HttpServletRequest request){
		...
		// rï¿½cupï¿½re le nom du chat ï¿½ chercher
		String nomChat = request.getParameter("RechAstucesChat");
		//indique la page ï¿½ retourner
		...
		//trouver le chat
          ...
          if (chat != null) {
        	  //Si le chat existe, on va chercher la mï¿½thode qui va chercher pour nous
		...
		modelAndView.addObject("error", "Il n'y a pas d'astuces associï¿½es ï¿½ ce chat");
		return modelAndView;
          }
...
public ModelAndView AstucesAssocieesAuChatNORMAL(int idChat,HttpSession httpSession){
	...
	public ModelAndView AstucesAssocieesAuChat(@PathVariable("idChat") int idChat,HttpSession httpSession){
    	...
    	criteria.add(Restrictions.eq("chat",chat))
    			.add(Restrictions.eq("publie",  true));
		List<Astuce> astuces = (List<Astuce>)criteria.list();      
		for (int i = 1 ; i < astuces.size(); i++)
			System.out.println("Astuce :"+astuces.get(i).getAstuce());
		session.close();
	...
	public ModelAndView AjoutAstuce(@ModelAttribute("astuce") Astuce astuce, BindingResult result,@PathVariable("idChat") int idChat,HttpSession httpSession){
	 ...
	 public ModelAndView cacherAstuce(@PathVariable("idAstuce") int idAstuce,HttpSession httpSession){
		 ...
		 SessionFactory sessionFactory = new Configuration().configure().buildSessionFactory();
			...
			ModelAndView modelAndView = affichePanneauAdminAstuces(httpSession);
			modelAndView.addObject("Succes","L'astuce a ete cachÃ©e avec succes.");
